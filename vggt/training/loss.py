...
huber_loss = nn.HuberLoss(reduction='none', delta=0.075)
...
def camera_loss(pred_pose_enc_list, batch, loss_type="l1", gamma=0.6, pose_encoding_type="absT_quaR_FoV", weight_T = 1.0, weight_R = 1.0, weight_fl = 0.5, frame_num = -100, avg_scale=None):
    # Extract predicted and ground truth components
    mask_valid = batch['point_masks']
    
    batch_valid_mask = mask_valid[:, 0].sum(dim=[-1, -2]) >= 0
    # Mask Agibot World dataset for camera loss
    dataset_camera_mask = torch.tensor(([not dataset == 'agibot_world' for dataset in batch['dataset_name']]), dtype=torch.bool, device=mask_valid.device)
    # print(dataset_camera_mask.shape)
    batch_dataset_valid_mask = batch_valid_mask & dataset_camera_mask
    num_predictions = pred_pose_enc_list.shape[1]
    ...
    for i in range(num_predictions):
        ...
        if frame_num>0:
            ...
            loss_T_i, loss_R_i, loss_fl_i = camera_loss_single(cur_pred_pose_enc[:, :frame_num].clone(), gt_pose_encoding[:, :frame_num].clone(), loss_type=loss_type, avg_scale=avg_scale)
        else:
            # loss_T_i, loss_R_i, loss_fl_i = camera_loss_single(cur_pred_pose_enc[batch_valid_mask].clone(), gt_pose_encoding[batch_valid_mask][:, i].clone(), loss_type=loss_type)
            loss_T_i, loss_R_i, loss_fl_i = camera_loss_single(cur_pred_pose_enc.clone(), gt_pose_encoding[:, i].clone(), loss_type=loss_type, avg_scale=avg_scale)
        
        if batch_valid_mask.sum() == 0:
            loss_T += (cur_pred_pose_enc * 0).mean()
            loss_R += (cur_pred_pose_enc * 0).mean()
            loss_fl += (cur_pred_pose_enc * 0).mean()
        elif batch_dataset_valid_mask.sum() == 0:
            loss_T += (cur_pred_pose_enc * 0).mean()
            loss_R += (cur_pred_pose_enc * 0).mean()
            loss_fl += (cur_pred_pose_enc * 0).mean()
        else:
            loss_T += loss_T_i[batch_dataset_valid_mask].mean() * i_weight
            loss_R += loss_R_i[batch_dataset_valid_mask].mean() * i_weight
            loss_fl += loss_fl_i[batch_dataset_valid_mask].mean() * i_weight
...
def camera_loss_single(cur_pred_pose_enc, gt_pose_encoding, loss_type="l1", avg_scale=None):
    if avg_scale is not None:
        # normalize the translation part of the pose encoding
        gt_pose_encoding[..., :3] = gt_pose_encoding[..., :3] / avg_scale.view(-1, 1)
    if loss_type == "l1":
...
def normalize_pointcloud(pts3d, valid_mask, eps=1e-3, avg_scale=None):
    """
    pts3d: B, S, H, W, 3
    valid_mask: B, S, H, W
    """
...
def depth_loss(depth, depth_conf, batch, gamma=1.0, alpha=0.2, loss_type="conf", predict_disparity=False, affine_inv=False, gradient_loss= None, valid_range=-1, disable_conf=False, all_mean=False, avg_scale=None, **kwargs):
    ...
    if loss_type == "conf":
        conf_loss_dict, avg_scale = conf_loss(depth, depth_conf, gt_depth, valid_mask,
                               batch, normalize_pred=False, normalize_gt=True,
                               gamma=gamma, alpha=alpha, affine_inv=affine_inv, gradient_loss=gradient_loss, valid_range=valid_range, postfix="_depth", disable_conf=disable_conf, all_mean=all_mean, avg_scale=avg_scale)
    else:
        raise ValueError(f"Invalid loss type: {loss_type}")
    return conf_loss_dict, avg_scale


def metric_depth_loss(depth, depth_conf, batch, gamma=1.0, alpha=0.2, loss_type="conf", predict_disparity=False, affine_inv=False, gradient_loss= None, valid_range=-1, disable_conf=False, all_mean=False, **kwargs):
    ...
    if loss_type == "conf":
        conf_loss_dict, _ = conf_loss(depth, depth_conf, gt_depth, valid_mask,
...
def point_loss(pts3d, pts3d_conf, batch, normalize_pred=False, gamma=1.0, alpha=0.2, affine_inv=False, gradient_loss=None, valid_range=-1, camera_centric_reg=-1, disable_conf=False, all_mean=False, conf_loss_type="v1", avg_scale=None, **kwargs):
    """
    pts3d: B, S, H, W, 3
    pts3d_conf: B, S, H, W
    """
    ...
    # valid_mask: B, S, H, W
    valid_mask = batch['point_masks']
    # mask out AgiBot World dataset for point loss
    dataset_mask = torch.tensor(([not dataset == 'agibot_world' for dataset in batch['dataset_name']]), dtype=torch.bool, device=valid_mask.device).view(-1, 1, 1, 1)
    valid_mask = valid_mask & dataset_mask
    ...
    conf_loss_dict, avg_scale = conf_loss_fn(pts3d, pts3d_conf, gt_pts3d, valid_mask,
                                batch, normalize_pred=normalize_pred, gamma=gamma, alpha=alpha, affine_inv=affine_inv,
                                gradient_loss=gradient_loss, valid_range=valid_range, camera_centric_reg=camera_centric_reg, disable_conf=disable_conf, all_mean=all_mean, avg_scale=avg_scale)


    return conf_loss_dict, avg_scale

def metric_point_loss(pts3d, pts3d_conf, batch, gamma=1.0, alpha=0.2, affine_inv=False, gradient_loss=None, valid_range=-1, camera_centric_reg=-1, disable_conf=False, all_mean=False, conf_loss_type="v1", **kwargs):
    """
    pts3d: B, S, H, W, 3
    pts3d_conf: B, S, H, W
    """
    ...
    conf_loss_dict, _ = conf_loss_fn(pts3d, pts3d_conf, gt_pts3d, valid_mask,
                                batch, normalize_pred=False, normalize_gt=False, gamma=gamma, alpha=alpha, affine_inv=affine_inv,
...
def conf_loss(pts3d, pts3d_conf, gt_pts3d, valid_mask,  batch, normalize_gt=True, normalize_pred=True, gamma=1.0, alpha=0.2, affine_inv=False, gradient_loss=None, valid_range=-1, camera_centric_reg=-1, disable_conf=False, all_mean=False, avg_scale=None, postfix=""):
    # normalize
    if normalize_gt:
        gt_pts3d, avg_scale = normalize_pointcloud(gt_pts3d, valid_mask, avg_scale=avg_scale)

    if normalize_pred:
        pts3d, avg_scale = normalize_pointcloud(pts3d, valid_mask, avg_scale=avg_scale)
    ...
    if conf_loss_first_frame.numel() >0 and conf_loss_other_frames.numel() >0:
        ...
        print(batch["point_masks"])
        print("No valid conf loss", batch["seq_name"], batch["ids"])
    """
    if conf_loss_first_frame.numel() >0:
        if valid_range>0:
            conf_loss_first_frame = filter_by_quantile(conf_loss_first_frame, valid_range)
        conf_loss_first_frame = check_and_fix_inf_nan(conf_loss_first_frame, f"conf_loss_first_frame{postfix}")
    else:
        conf_loss_first_frame = pts3d * 0
        # print("No valid conf loss", batch["seq_name"], batch["ids"])
    if conf_loss_other_frames.numel() >0:
        if valid_range>0:
            conf_loss_other_frames = filter_by_quantile(conf_loss_other_frames, valid_range)
        conf_loss_other_frames = check_and_fix_inf_nan(conf_loss_other_frames, f"conf_loss_other_frames{postfix}")
    else:
