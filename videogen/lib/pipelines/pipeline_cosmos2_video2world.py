...
from lib.models import AutoencoderKLWan, CosmosTransformer3DModel
...
class Cosmos2VideoToWorldPipeline(DiffusionPipeline):
    ...
    @classmethod
    def from_pretrained(cls, model_id="nvidia/Cosmos-Predict2-2B-Video2World", torch_dtype=torch.bfloat16, device='cuda'):
        text_encoder = T5EncoderModel.from_pretrained(osp.join(model_id, 'text_encoder'), torch_dtype=torch_dtype)
        tokenizer = T5TokenizerFast.from_pretrained(osp.join(model_id, 'tokenizer'))
        vae = AutoencoderKLWan.from_pretrained(osp.join(model_id, 'vae'), torch_dtype=torch_dtype)
        scheduler = FlowMatchEulerDiscreteScheduler.from_pretrained(osp.join(model_id, 'scheduler'))
        transformer = CosmosTransformer3DModel.from_pretrained(osp.join(model_id, 'transformer'), torch_dtype=torch_dtype)

        pipe = cls(text_encoder, tokenizer, transformer, vae, scheduler)
        pipe.to(device)
        return pipe